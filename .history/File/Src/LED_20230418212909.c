            ...
            for (rowcounter=0 ; rowcounter<(16) ; rowcounter++) 
            {
                ...
                for (colcounter=0 ; colcounter<32 ; colcounter++) 
                {
                   ...
                   // sColorCode = (sColorCode > 250) ? 0 : sColorCode;
                    ...
                    if (((m_ColorCode_256[sColorCode10][1] >> pwmindex) & 0x01)==1) HAL_GPIO_WritePin(LED_DATA_Port,LED_DATA_G_2,GPIO_PIN_SET);
                    else HAL_GPIO_WritePin(LED_DATA_Port,LED_DATA_G_2,GPIO_PIN_RESET);

                    if (((m_ColorCode_256[sColorCode10][2] >> pwmindex) & 0x01)==1) HAL_GPIO_WritePin(LED_DATA_Port,LED_DATA_B_2,GPIO_PIN_SET);
