...
void LED_GPIO_Init(void)
{
	...
	GPIO_InitStructure.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStructure.Pull = GPIO_PULLUP;
	GPIO_InitStructure.Pin = LED_DATA_R_1,LED_DATA_G_1,LED_DATA_B_1,LED_DATA_R_2,LED_DATA_B_2,LED_DATA_G_2; 
	GPIO_InitStructure.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
    ...
    if(dTest_Print_Flag)
    {
      dTest_Print_Flag = 0;
        ...
        else if(LED_Imag_ScreenNormal_Cnt()) // 노말 버번 출력 기능.
        {
         ...
         HAL_GPIO_WritePin(LED_DIS_Port, LED_DIS, GPIO_PIN_RESET);  // OE - High
            ...
            for (rowcounter=0 ; rowcounter<(16) ; rowcounter++) 
            {
                ...
                for (colcounter=0 ; colcounter<32 ; colcounter++) 
                {
                   ...
                   // sColorCode = pVd[sColorCode_Cnt];
                    ...
                    if (((m_ColorCode_256[sColorCode][1] >> pwmindex) & 0x01)==1) HAL_GPIO_WritePin(LED_DATA_Port,LED_DATA_G_1,GPIO_PIN_SET);
                    else HAL_GPIO_WritePin(LED_DATA_Port,LED_DATA_G_1,GPIO_PIN_RESET);

                    if (((m_ColorCode_256[sColorCode][2] >> pwmindex) & 0x01)==1) HAL_GPIO_WritePin(LED_DATA_Port,LED_DATA_B_1,GPIO_PIN_SET);
                ...
                HAL_GPIO_WritePin(LED_ADD_Port, LED_ADD_3, ((rowcounter & 0x04) ? GPIO_PIN_SET : GPIO_PIN_RESET));  //A2      
                ...
                HAL_GPIO_WritePin(LED_OE_Port, LED_OE_R, GPIO_PIN_RESET);  // OE - High
                HAL_GPIO_WritePin(LED_OE_Port, LED_OE_G, GPIO_PIN_RESET);  // OE - High
                HAL_GPIO_WritePin(LED_OE_Port, LED_OE_B, GPIO_PIN_RESET);  // OE - High
        ...
        HAL_GPIO_WritePin(LED_DIS_Port, LED_DIS, GPIO_PIN_SET);  // OE - High
