...
uint8_t BSP_IO_Init(void)
{
  ...
  if(stmpe811_io_drv.ReadID(0x88) == 0x0811)
  {  
     
    io_driver = &stmpe811_io_drv;
    
    io_driver->Init(0x88);
    io_driver->Start(0x88, 0xFF);
...
uint8_t BSP_IO_ITGetStatus(uint16_t IO_Pin)
{
  ...
  return (io_driver->ITStatus(0x88, IO_Pin));
}
...
uint8_t BSP_IO_ConfigPin(uint16_t IO_Pin, IO_ModeTypedef IO_Mode)
{
  ...
  io_driver->Config(0x88, IO_Pin, IO_Mode);    
...
void BSP_IO_WritePin(uint16_t IO_Pin, uint8_t PinState)
{
  io_driver->WritePin(0x88, IO_Pin, PinState);
}
...
uint16_t BSP_IO_ReadPin(uint16_t IO_Pin)
{
  return(io_driver->ReadPin(0x88, IO_Pin));
}
...
void BSP_IO_TogglePin(uint16_t IO_Pin)
{
  if(io_driver->ReadPin(0x88, IO_Pin) == 1)  
  {
    io_driver->WritePin(0x88, IO_Pin, 0);    
  }
  else
  {
    io_driver->WritePin(0x88, IO_Pin, 1);    
  } 
}
